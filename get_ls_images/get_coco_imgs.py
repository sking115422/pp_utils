...
"""
This script will gather all the images from the server that correspond to the coco file that script is pointed at.

*** Make sure to remove the "_" from coco and imgs directories in the same parent directory of this script. 
"""
...
with open('./coco/result.json') as f:
    res_obj = json.load(f)
...
for each in res_obj["images"]:
    name = each["file_name"].split("/")[-1]
